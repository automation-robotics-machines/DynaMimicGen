...
if __name__ == "__main__":
    ...
    parser.add_argument("--demo", type=str, default="demo_4", help="Name of the demo to analyze")
    ...
    # === 1️⃣  Convert ABS → DELTA → ABS ===
    delta_from_abs = utils.convert_absolute_to_delta(abs_actions)
    abs_reconstructed_from_abs = utils.convert_delta_to_absolute(delta_from_abs, init_pose=abs_actions[0])

    # === 2️⃣  Convert DELTA → ABS → DELTA ===
    abs_from_delta = utils.convert_delta_to_absolute(delta_actions)
    delta_reconstructed_from_delta = utils.convert_absolute_to_delta(abs_from_delta)
    ...
    # === Compute alignment for plotting ===
